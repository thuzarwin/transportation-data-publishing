...
class Gridsmart:
    ...
    fieldmap_zones : dict (default: None)
        a dict of format { input_key : output_key }
        when provided, keys in zone data will be replaced according to field mapwhen provided, keys in zone data will be replaced according to field map
    '''
        ...
        if get_zones:
            ...
            if fieldmap_zones:
                for zone in self.zones:
                    self.zones[zone] = self.mapFields(self.zones[zone])

        self.days_available = None
        self.getDaysAvailable()
    ...
    def getName(self):
        ...
        value : string
            The site name
        '''
        ep = self.endpoint + 'sitename.json'
        res = requests.get(ep, timeout=self.timeout)
        if res.status_code != 200:
    ...
    def getZones(self):
        ...
        site_id = dashify_site_or_zone_id(self.config['Id'])
        config['Id'] = site_id
        ...
        **Keys with empty values are dropped**
        ...
        for key in obj.keys():
            ...
            if key in self.fieldmap:
                obj[self.fieldmap[key]] = obj.pop(key)
            else:
    ...
    def getCounts(self, day_str, local_file = None):
        '''
        Download the requested day of data to the location specified.
        ...
        '''
        ep = self.endpoint + 'counts/bydate/' + day_str
        ...
        r = requests_get_by_chunks(ep, local_file)
